...
3x3x3 Solver / Scramble Generator in Python.

Ported to python from javascript by gyroninja
...
class scrambler333:
    ...
    def createArray(length1, length2):
        result = Array(length1);
        for i in range(length1):
            result[i] = Array(legnth2);
        return result;

    def clinit_CoordCube():
    ...
    def initMCEPermPrun(callback):
        ...
        for i in range(2768):
            ...
            for j in range(16):
                ...
                if binarySearch(EPermS2R, get8Perm(d.ep)) != 65535:
        ...
        while (done < 66432):
            inv = depth > 7;
            select = -1 if inv else depth;
            check = depth if inv else -1;
            ++depth;
            for i in range(66432):
    ...
    def initTwistFlipSlicePrun(callback):
        SymState = Array(324);
        c = CubieCube_0();
        d = CubieCube_0();
        for i in range(324):
            ...
            for j in range(8):
                ...
                if (binarySearch(TwistS2R, getTwist(d)) != 65535):
                    SymState[i] = SymState[i] | (1 << j);
        SymStateF = Array(336);
        for i in range(336):
            SymStateF[i] = 0;
            ...
            for j in range(1, 8):
                ...
                if (binarySearch(FlipS2R, getFlip(d)) != 65535):
                    SymStateF[i] = SymStateF[i] | (1 << j);
        for i in range(870912):
            TwistFlipPrun[i] = -1;
        for i in range(8):
            TwistFlipPrun[i] = 0;
        depth = 0;
...
function randomCube_0(){
  var cperm, eperm;
  do {
    eperm = random.randint(0, 479001600);
    cperm = random.randint(0, 40320);
  }
   while ((get8Parity(cperm) ^ get12Parity(eperm)) != 0);
  return toFaceCube(new CubieCube_2(cperm, random.randint(0, 2187), eperm, random.randint(0, 2048)));
}
